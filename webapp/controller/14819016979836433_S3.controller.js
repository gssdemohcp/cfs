sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("generated.app.controller.14819016979836433_S3", {

		onInit: function() {
			//###################################### STANDARD CODE ####################################################
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getTarget("14819016979836433_S3").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
			//#########################################################################################################
			...
			//================================================
			//Array Data For the Table Sales Document Search
			//================================================
			var oJSONModel = new JSONModel(SalesDocumentSearch);
			this.getView().byId(
					"table")
				.setModel(
					oJSONModel);
			...
			//=======================
			var filters = [];
			var Search = SaleDocSearch;
			if (Search) {
				filters = [new sap.ui.model.Filter([new sap.ui.model.Filter("VBELN", sap.ui.model.FilterOperator.Contains, Search)])];
			}
		...
		handleClose: function(oEvent) {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Document Type") {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Sold To Party") {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Credit Status") {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Overall Status ID") {
		...
		//===================================
		rowCount: function() {
			if (SalesDocumentSearch.SalesDocumentSearch.length <= 7) {
				this.getView().byId("table").setVisibleRowCount(SalesDocumentSearch.SalesDocumentSearch.length);
			} else {
				this.getView().byId("table").setVisibleRowCount(7);
			}
		},
		//************* SRINI 01/02/2017 - CODE ENDS *************************************
		...
		handleRouteMatched: function(oEvent) {
				...
				var Search = SaleDocSearch;
				if (Search) {
					filters = [new sap.ui.model.Filter([new sap.ui.model.Filter("ID", sap.ui.model.FilterOperator.Contains, Search)])];
				}
				//=====================
				// update list binding
				//=====================
