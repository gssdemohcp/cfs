sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("generated.app.controller.14819014925569885_S1", {
		onInit: function(evt) {
			//###################################### STANDARD CODE ####################################################
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getTarget("14819014925569885_S1").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
			//#########################################################################################################
			...
			var oData = {
				"d": {
					"results": [{
						...
						"Stext": "Sales Doc Type"
					}, {
						"Stext": "Sold To Party ID"
					}, {
						"Stext": "Sold To Party Name"
					}, {
						"Stext": "Created On"
					}]
				}
		...
		handleClose: function(oEvent) {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Sales Doc Type") {
			...
			for (var i = 0; i < column_selected.length; i++) {
				if (column_selected[i] === "Sold To Party Name") {
		...
		rowCount: function() {
			if (Last10.last10.length <= 7) {
				this.getView().byId("table").setVisibleRowCount(Last10.last10.length);
			} else {
		...
		JSONToPDFConvertor: function(JSONData) {
			...
			doc.text("Order Tracking", 7, doc.autoTableEndPosY() + 10);
			...
			doc.text("Deliveries", 7, doc.autoTableEndPosY() + 10);
		...
		getContextByIndex: function() {
			var iIndex = this.getView().byId("table").getSelectedIndex();
			var sMsg;
			if (iIndex < 0) {
				sMsg = "no item selected";
			} else {
				sMsg = this.getView().byId("table").getSelectedIndex(iIndex);
			}
			MessageToast.show(sMsg);
		},
